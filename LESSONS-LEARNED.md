...
### [Testing] — 2026-08-29
- **MISTAKE**: `AUTONOMOUS-BUILD.md` and `scripts/verify-adam-task.sh` both gated this build with bare `npx vitest run <paths>` from `dobius/`. **That command loads NO config.** Vitest looks for `vitest.config.ts` in the cwd; this project keeps it at `config/vitest.config.ts` and every real entry point passes it explicitly (`package.json` → `"test": "… vitest run --config config/vitest.config.ts"`). There is no config file at `dobius/` root, so the bare command silently ran with Vitest defaults.
- **SYMPTOM**: intermittent second and third failing files in `src/main/window` (`tear-off-window.test.ts`, `createMainWindow.test.ts`) — which the build file defines as "you broke something". They pass in isolation and fail perhaps 1 run in 5, with the set of failing files changing between runs.
- **ROOT CAUSE**: the real error was `Error: Test timed out in 5000ms` on `await import('./tear-off-window')`. The project config sets `testTimeout: 30_000` precisely because these files pull a heavy module graph (electron, pty) through a cold TS transform — total transform time for the scoped set is ~30s. Without the config the timeout falls back to the 5s default, so any dynamic import unlucky enough to land under worker contention loses. **Not** module/mock contamination, which was the first (wrong) hypothesis — the captured error message disproved it.
