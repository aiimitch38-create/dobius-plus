...
const iris = agent("Iris", "builtin:iris", "f".repeat(64));
const atlas = agent("Atlas", "builtin:atlas", "h".repeat(64));
const sage = agent("Sage", "builtin:sage", "b".repeat(64));

test("resolveWelcomeAgentSet orders agents by stable persona identity", () => {
  assert.deepEqual(resolveWelcomeAgentSet([sage, iris, atlas]), {
    lead: iris,
    teammates: [atlas, sage],
  });
  assert.equal(resolveWelcomeAgentSet([iris, atlas]), null);
});

test("opener uses current agent names and requests bounded simultaneous intros", () => {
  const opener = buildWelcomeKickoffOpener({ ...iris, name: "Fizzy" }, [
    { ...atlas, name: "Honeybee" },
    sage,
  ]);

  assert.match(opener, /I'm Fizzy/);
  assert.match(opener, /@Honeybee and @Sage/);
...
test("teammates are not ready until every harness publishes online presence", () => {
  assert.equal(areWelcomeTeammatesOnline([atlas, sage], undefined), false);
  assert.equal(
    areWelcomeTeammatesOnline([atlas, sage], {
      [atlas.pubkey]: "online",
      [sage.pubkey]: "offline",
    }),
    false,
...
test("readiness wait observes agents becoming online without navigation", async () => {
  let reads = 0;
  const ready = await waitForWelcomeTeammatesOnline([atlas, sage], {
    isCancelled: () => false,
    loadPresence: async () => {
      reads += 1;
      return reads < 3
        ? { [atlas.pubkey]: "online", [sage.pubkey]: "offline" }
        : { [atlas.pubkey]: "online", [sage.pubkey]: "online" };
    },
    pollMs: 0,
    waitMs: 1_000,
  });

  assert.deepEqual(ready, [atlas, sage]);
...
test("readiness wait retries transient presence failures", async () => {
  ...
  const ready = await waitForWelcomeTeammatesOnline([atlas, sage], {
    ...
    loadPresence: async () => {
      ...
      return { [atlas.pubkey]: "online", [sage.pubkey]: "online" };
    },
    pollMs: 0,
    waitMs: 1_000,
  });
...
test("closer degrades coherently for partial and total startup failure", () => {
  ...
  assert.match(buildWelcomeKickoffCloser(["Atlas"]), /Atlas is having trouble/);
  assert.match(
    buildWelcomeKickoffCloser(["Atlas", "Sage"]),
    /Atlas and Sage couldn't start/,
  );
  assert.match(
    buildWelcomeKickoffCloser(["Atlas", "Sage"]),
...
test("closer names teammates that did not reply before the intro wait", () => {
  assert.match(
    buildWelcomeKickoffCloser([], ["Sage"]),
    /Sage is taking longer to reply/,
  );
  assert.match(
    buildWelcomeKickoffCloser(["Atlas"], ["Sage"]),
    /Atlas and Sage are taking longer than expected/,
  );
});
    ...
    welcomeTeammateNeedsRestart(
      {
        ...
        status: "running",
        respondTo: "allowlist",
        respondToAllowlist: [iris.pubkey],
      },
      iris.pubkey,
    ),
    false,
    ...
    welcomeTeammateNeedsRestart(
      {
        ...
        status: "running",
        respondTo: "allowlist",
        respondToAllowlist: [atlas.pubkey],
      },
      iris.pubkey,
    ),
    true,
...
test("opener keeps partial-readiness warm and mentions only online teammates", () => {
  const agentSet = { lead: iris, teammates: [atlas, sage] };
  ...
  assert.deepEqual(input.mentionPubkeys, [atlas.pubkey]);
  assert.deepEqual(input.additionalMarkers, []);
  assert.match(input.content, /@Atlas, introduce yourself/);
  assert.doesNotMatch(input.content, /@@/);
  assert.doesNotMatch(
    input.content,
    /Sage.*trouble|couldn't start|taking longer/i,
  );
});
...
test("opener greets the owner by name and tags their pubkey", () => {
  ...
  assert.match(input.content, /^Hi @Morgan, I'm Iris\./);
...
test("opener falls back to an unnamed greeting when the display name is missing", () => {
  ...
  assert.match(input.content, /^Hi, I'm Iris\./);
...
test("opener does not duplicate the owner pubkey if already mentioned", () => {
  ...
  const input = buildWelcomeKickoffOpenerSendInput(
    ...
    [atlas],
    "welcome-1",
    { pubkey: atlas.pubkey, displayName: atlas.name },
  );

  assert.deepEqual(input.mentionPubkeys, [atlas.pubkey]);
});

test("opener degrades to one seeded Iris message when no teammate comes online", () => {
  const agentSet = { lead: iris, teammates: [atlas, sage] };
  const input = buildWelcomeKickoffOpenerSendInput(agentSet, [], "welcome-1");

  assert.deepEqual(input.mentionPubkeys, []);
  assert.equal(input.additionalMarkers.length, 1);
  assert.match(input.content, /I'm here with Atlas and Sage/);
...
test("readiness wait returns the subset that became online by the deadline", async () => {
  const online = await waitForWelcomeTeammatesOnline([atlas, sage], {
    isCancelled: () => false,
    loadPresence: async () => ({
      [atlas.pubkey]: "online",
      [sage.pubkey]: "offline",
...
test("intro replies reach the closer classification without the user opening the thread", () => {
  ...
  const openerReplies = [
    introReply("atlas-intro", atlas.pubkey, kickoffOpener.id),
    introReply("sage-intro", sage.pubkey, kickoffOpener.id),
  ];
...
test("merging the opener subtree never double-counts an already-visible reply", () => {
  const honeyIntro = introReply("atlas-intro", atlas.pubkey, kickoffOpener.id);
  // An open thread feeds the same replies in through both sources.
  const merged = mergeKickoffEvents(
    [kickoffOpener, honeyIntro],
    [honeyIntro, introReply("sage-intro", sage.pubkey, kickoffOpener.id)],
  );

  assert.deepEqual(
    merged.map((event) => event.id),
    ["opener", "atlas-intro", "sage-intro"],
  );
});
