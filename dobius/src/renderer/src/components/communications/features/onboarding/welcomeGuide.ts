...
import { getGlobalAgentConfig } from "@comms/shared/api/tauriGlobalAgentConfig";
import { listPersonas, setPersonaActive } from "@comms/shared/api/tauriPersonas";
import type {
  AcpRuntime,
  AgentPersona,
...
export const WELCOME_GUIDE_AGENT_NAME = "Iris";
export const WELCOME_GUIDE_PERSONA_ID = "builtin:iris";
...
export const WELCOME_GUIDE_INTRO_MESSAGE =
  "Hi, I'm Iris. Welcome to Dobius.\n\nI can help you get oriented, answer questions, and make the first few steps feel less mysterious.\n\nFeel free to ask me what else you can do in Dobius, or just talk through what you want to build.";
...
/** Legacy Iris/Kit lookup retained for existing channel reuse checks. */
...
async function provisionWelcomeTeam(
  ...
  const [lead, atlas, sage] = agents;
  if (!lead || !atlas || !sage) {
    throw new Error("Welcome Team provisioning did not return every starter.");
  }
  const welcomeAgents: WelcomeTeamAgents = [lead, atlas, sage];
