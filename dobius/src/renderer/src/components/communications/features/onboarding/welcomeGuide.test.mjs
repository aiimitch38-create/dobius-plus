...
test("pickWelcomeGuideAgent prefers a running legacy guide over stopped builtin Iris", () => {
...
test("pickWelcomeGuideAgent ignores non-Kit agents with the legacy prompt", () => {
  ...
  const iris = makeAgent({
    pubkey: PUB_C,
    personaId: WELCOME_GUIDE_PERSONA_ID,
  });

  assert.equal(pickWelcomeGuideAgent([nonKit, iris]), iris);
});

test("pickWelcomeGuideAgentForRelay ignores Iris agents from other communities", () => {
...
test("pickWelcomeGuideAgentForRelay returns null when Iris only exists in another community", () => {
  const otherCommunityFizz = makeAgent({
    pubkey: PUB_A,
    personaId: WELCOME_GUIDE_PERSONA_ID,
    relayUrl: RELAY_A,
...
test("starter persona activation is serialized to protect the shared store", async () => {
  ...
  assert.deepEqual(calls, ["builtin:iris", "builtin:atlas", "builtin:sage"]);
});
...
test("welcome team starter definitions and role identities are stable", () => {
  ...
  assert.deepEqual(WELCOME_TEAM_STARTERS, [
    { name: "Iris", personaId: "builtin:iris", role: "lead" },
    { name: "Atlas", personaId: "builtin:atlas", role: "teammate" },
    { name: "Sage", personaId: "builtin:sage", role: "teammate" },
  ]);
});

test("starter matching ignores user agents with a Welcome persona", () => {
  const atlas = WELCOME_TEAM_STARTERS[1];
  const userHoney = makeAgent({
    personaId: atlas.personaId,
    teamId: null,
  });

  assert.equal(
    pickWelcomeTeamStarterAgentForRelay([userHoney], atlas, RELAY_A),
    null,
  );
});
...
test("starter matching uses persona identity rather than display name", () => {
  ...
  const renamedHoney = makeAgent({
    name: "Atlas the Helper",
    personaId: atlas.personaId,
  });
  const nameOnlyHoney = makeAgent({ name: atlas.name, pubkey: PUB_B });
...
test("starter matching is relay scoped and normalizes trailing slashes", () => {
  const sage = WELCOME_TEAM_STARTERS[2];
  const otherRelay = makeAgent({
    personaId: sage.personaId,
    relayUrl: RELAY_B,
    status: "running",
  });
...
test("starter matching prefers running, then deployed instances", () => {
  const iris = WELCOME_TEAM_STARTERS[0];
  const stopped = makeAgent({ personaId: iris.personaId });
  const deployed = makeAgent({
    personaId: iris.personaId,
    pubkey: PUB_B,
    status: "deployed",
  });
  ...
  assert.equal(
    pickWelcomeTeamStarterAgentForRelay([stopped, deployed], iris, RELAY_A),
    deployed,
  );
});
