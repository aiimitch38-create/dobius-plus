...
import type { SoundName, SoundSlot } from "@comms/features/notifications/lib/sound";
import { CommunityMembersSettingsCard } from "@comms/features/community-members/ui/CommunityMembersSettingsCard";
import { CustomEmojiSettingsCard } from "@comms/features/custom-emoji/ui/CustomEmojiSettingsCard";
import { LocalArchiveSettingsCard } from "@comms/features/local-archive/ui/LocalArchiveSettingsCard";
import {
  setThreadViewMode,
  useThreadViewMode,
  type ThreadViewMode,
} from "@comms/features/channels/lib/threadViewModePreference";
...
import {
  ...
  NEUTRAL_ACCENT,
  useTheme,
} from "@comms/shared/theme/ThemeProvider";
import {
  LIGHT_THEMES,
  SYNTAX_THEMES,
  type SyntaxThemeName,
  getThemePair,
} from "@comms/shared/theme/theme-loader";
import {
  BUZZ_GRADIENT_STOPS,
  SystemPreferencePreviewFrame,
  ThemePreviewFrame,
  type ThemePreviewVars,
} from "@comms/shared/theme/ThemePreviewFrame";
import {
  getThemeFallbackPreviewVars,
  useThemePreviewVars,
  withAccentPreviewVars,
} from "@comms/shared/theme/useThemePreviewVars";
import { ChannelTemplatesSettingsCard } from "./ChannelTemplatesSettingsCard";
import { HarnessesSettingsPanel } from "./HarnessesSettingsPanel";
import { ExperimentalFeaturesCard } from "./ExperimentalFeaturesCard";
import { KeyboardShortcutsCard } from "./KeyboardShortcutsCard";
import { MeshComputeSettingsCard } from "@comms/features/mesh-compute/ui/MeshComputeSettingsCard";
...
// it enters from above (slides *down* into place when a non-Dobius theme reveals
// it) and exits upward (slides up behind the grid when Dobius hides it). No
...
function ThemeSettingsCard() {
  ...
  // Dobius themes pin a neutral accent (GitHub black in light, white in dark),
  // so the accent picker is hidden while a Dobius theme is active. `themeName` is
  // the effective theme, so this also covers System mode resolving to Dobius.
  ...
  return (
    ...
    >
      <SettingsSectionHeader
        title="Appearance"
        description="Choose a theme for Dobius."
      />
      ...
      <div className="relative mb-6">
        ...
        {/* Bottom fade — hidden while the accent picker is visible so its
            near-white gradient (Dobius light) can't mask the swatches below it
      ...
      {/* Accent color picker — hidden for Dobius themes (pinned neutral accent).
