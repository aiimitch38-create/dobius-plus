...
import type { ProjectsWorkItemsResult } from "@comms/features/projects/projectWorkItems";
import type { FeedItem } from "@comms/shared/api/types";
import {
  KIND_GIT_ISSUE,
  KIND_GIT_PR_UPDATE,
...
/** Returns the canonical Dobius Git repository and root event for an Inbox row. */
...
/** Whether a feed event belongs to a Dobius Git pull request or issue thread. */
export function isProjectInboxItem(item: FeedItem) {
  return getProjectInboxReference(item) !== null;
}

/** Resolves an Inbox event to the current canonical Dobius Git work item. */
