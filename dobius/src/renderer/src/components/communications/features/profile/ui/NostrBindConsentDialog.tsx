...
import type { Identity } from "@comms/shared/api/types";
import type { NostrBindDeepLinkPayload } from "@comms/shared/deep-link";
import { listenForNostrBindDeepLinks } from "@comms/shared/deep-link";
import { OnboardingSlideTransition } from "@comms/features/onboarding/ui/OnboardingSlideTransition";
import { buildNostrBindCallbackUrl } from "@comms/features/profile/lib/nostrBindCallback";
import { signNostrIdentityBinding } from "@comms/features/profile/lib/nostrIdentityBinding";
...
const COPY_SUCCESS_MESSAGE =
  "Signed response copied. Paste it into the Dobius admin console.";
const PREVIEW_COPY_SUCCESS_MESSAGE = "Preview response copied.";
const COPY_FAILURE_MESSAGE = "Dobius couldn't access the clipboard. Try again.";
    ...
    const unlistenPromise = listenForNostrBindDeepLinks((nextPayload) => {
      ...
      getIdentity()
        ...
        .catch((error) => {
          ...
          setIdentity(null);
          setError("Could not load the current Dobius identity.");
        });
    });
            ...
            <div className="m-auto flex w-full max-w-[500px] flex-col items-center text-center">
              ...
              {signedResponse ? (
                ...
                >
                  ...
                  >
                    {payload.returnMode === "browser_fragment_v1"
                      ? "Dobius opened your browser to finish verification."
                      : "Copy the response below, then paste it into the Dobius website to finish verification."}
                  </DialogPrimitive.Description>
