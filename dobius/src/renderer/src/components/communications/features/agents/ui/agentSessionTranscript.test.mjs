...
test("buildTranscript renders Prompt context + user message for a multi-block session/prompt frame", () => {
  ...
  assert.deepEqual(
    ...
    ["Agent Memory — core", "Context", "Dobius event: @mention"],
...
test("buildTranscript keeps read_file activity categorized by the actual tool when output names Dobius tools", () => {
...
test("buildTranscript keeps shell activity categorized by the actual tool when grep output names Dobius tools", () => {
  const [item] = toolItems([
    acpToolUpdate(20, {
      sessionUpdate: "tool_call",
...
test("buildTranscript categorizes explicit Dobius tool calls for the activity bar", () => {
  const [item] = toolItems([
    acpToolUpdate(30, {
      sessionUpdate: "tool_call",
...
test("buildTranscript five-section system prompt card is standalone with all sections; CheckCheck context contains only Dobius/thread context", () => {
  ...
  //       the session/prompt:context sections (Dobius event + Thread context),
  ...
  // the session/prompt:context item — Dobius/thread context only, no system-prompt sections.
  ...
  // Must have Dobius event and Thread context sections, NOT Base/System/Team Instructions/Core Memory/Channel Canvas.
  assert.ok(
    contextSectionTitles.some((t) => t.toLowerCase().includes("buzz")),
    "prompt context must contain a Dobius event section",
  );
  assert.ok(
