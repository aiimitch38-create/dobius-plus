...
test("parsePromptText wraps header-less free text in a single Prompt section", () => {
  ...
  // section is a "Dobius event", there is no event content to surface, so
  // userText is empty and the title falls through to "Dobius event".
  ...
  assert.equal(result.userTitle, "Dobius event");
...
test("parsePromptText yields a null pubkey when From has no hex", () => {
  const text = ["[Dobius event: note]", "From: Someone", "Content: hi"].join(
    "\n",
  );
...
test("parsePromptText defaults the title to 'Dobius event' when no kind is present", () => {
  const text = ["[Dobius event]", "Content: x"].join("\n");
  const result = parsePromptText(text);
  assert.equal(result.userTitle, "Dobius event");
});
...
test("extractToolIdentity ignores Dobius tool names that only appear in file contents", () => {
...
test("extractToolIdentity still recognizes explicit Dobius tool fields", () => {
  const identity = extractToolIdentity({
...
test("parseSystemPromptSections pins the realistic Workspace+Base+System+Core harness shape", () => {
  // The real Dobius harness emits [Workspace] content before [Base]. The parser
  ...
  const framed = [
    "[Workspace]",
    "You are operating inside the Dobius platform.",
    "",
    "[Base]",
...
test("parseSystemPromptSections extracts Team Instructions with Core Memory and Channel Canvas (full 5-section shape)", () => {
  ...
  const framed = [
    "[Base]",
    "You are a helpful AI assistant running in Dobius.",
    "",
    "[System]",
  ...
  const sections = parseSystemPromptSections(framed);
  assert.deepEqual(sections, [
    { title: "Base", body: "You are a helpful AI assistant running in Dobius." },
    {
      title: "System",
