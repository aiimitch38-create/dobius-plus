...
import { deriveShellRoute } from "@comms/app/AppShell.helpers";
import { AppShellProvider } from "@comms/app/AppShellContext";
import * as BuzzTheme from "@comms/app/BuzzThemeSurfaces";
import { AppShellOverlays } from "@comms/app/AppShellOverlays";
import { AppTopChrome } from "@comms/app/AppTopChrome";
import { useAppNavigation } from "@comms/app/navigation/useAppNavigation";
import { useBackForwardControls } from "@comms/app/navigation/useBackForwardControls";
import { useCommunityNavigationTransitions } from "@comms/app/useCommunityNavigationTransitions";
import { useLiveHomeFeedActions } from "@comms/app/useLiveHomeFeedActions";
import { useChannelBrowserDialog } from "@comms/app/useChannelBrowserDialog";
import { useMarkAsReadShortcuts } from "@comms/app/useMarkAsReadShortcuts";
import { useSettingsShortcuts } from "@comms/app/useSettingsShortcuts";
import { useAppShellDesktopNotifications } from "@comms/app/useAppShellDesktopNotifications";
import { useAppShellLifecycleEffects } from "@comms/app/useAppShellLifecycleEffects";
import { useThreadActivityFeedItems } from "@comms/app/useThreadActivityFeedItems";
import { useTauriWindowDrag } from "@comms/app/useTauriWindowDrag";
import { useWebviewZoomShortcuts } from "@comms/app/useWebviewZoomShortcuts";
import {
  channelsQueryKey,
  useChannelsQuery,
  ...
  useHideDmMutation,
  useOpenDmMutation,
} from "@comms/features/channels/hooks";
import { useUnreadChannels } from "@comms/features/channels/useUnreadChannels";
import { msgContextKey } from "@comms/features/channels/readState/readStateFormat";
import { useMembershipNotifications } from "@comms/features/channels/useMembershipNotifications";
import { useFeedItemState } from "@comms/features/home/useFeedItemState";
import { useThreadFollows } from "@comms/features/messages/lib/useThreadFollows";
import {
  useHomeFeedNotifications,
  useHomeFeedNotificationState,
} from "@comms/features/notifications/hooks";
import { PreventSleepProvider } from "@comms/features/agents/usePreventSleep";
import { requestOpenCreateAgent } from "@comms/features/agents/openCreateAgentEvent";
import { useAgentsDataRefresh } from "@comms/features/agents/lib/useAgentsDataRefresh";
import { useManagedAgentRuntimeReconciliation } from "@comms/features/agents/useManagedAgentRuntimeReconciliation";
import { useAutoRestartPolicy } from "@comms/features/agents/lib/useAutoRestartPolicy";
import { usePersonaSync } from "@comms/features/agents/lib/usePersonaSync";
import { useAgentObserverIngestion } from "@comms/features/agents/useAgentObserverIngestion";
import { AgentManagementDialogs } from "@comms/features/agents/ui/AgentManagementDialogs";
import { RequestedAgentCreateDialogs } from "@comms/features/agents/ui/RequestedAgentCreateDialogs";
import {
  usePresenceSession,
  usePresenceSubscription,
} from "@comms/features/presence/hooks";
import {
  useSetUserStatusMutation,
  useUserStatusQuery,
  useUserStatusSubscription,
} from "@comms/features/user-status/hooks";
import { useCommunityEmojiLiveUpdates } from "@comms/features/custom-emoji/hooks";
import { useArchiveSync } from "@comms/features/local-archive/archiveSyncManager";
import { useObserverArchiveReconciliation } from "@comms/features/local-archive/useObserverArchiveSeed";
import { useAgentMetricArchiveSeed } from "@comms/features/local-archive/useAgentMetricArchiveSeed";
import { useProfileQuery } from "@comms/features/profile/hooks";
import { SendFeedbackController } from "@comms/features/settings/ui/SendFeedbackController";
import {
  DEFAULT_SETTINGS_SECTION,
  type SettingsSection,
  isSettingsSection,
} from "@comms/features/settings/ui/SettingsPanels";
import { HuddleBar, HuddleProvider } from "@comms/features/huddle";
import { useDueReminderBadgeCount } from "@comms/features/reminders/hooks";
import { RemindMeLaterProvider } from "@comms/features/reminders/ui/RemindMeLaterProvider";
import { useReminderNotifications } from "@comms/features/reminders/useReminderNotifications";
import { AppSidebar } from "@comms/features/sidebar/ui/AppSidebar";
import { requestFocusedThreadClose } from "@comms/features/channels/focusedThreadCloseRequest";
import { CommunityRail } from "@comms/features/sidebar/ui/CommunityRail";
import { useChannelMutes } from "@comms/features/sidebar/lib/useChannelMutes";
import { useChannelStars } from "@comms/features/sidebar/lib/useChannelStars";
import { useCommunities } from "@comms/features/communities/useCommunities";
import {
  consumePendingCommunityRestore,
  loadCommunityDestination,
  saveCommunityDestination,
} from "@comms/features/communities/communityNavigationStorage";
import { useAddCommunityDialogState } from "@comms/features/communities/addCommunityPrefill";
import { useApplyTemplate } from "@comms/features/channel-templates/useApplyTemplate";
import { relayClient } from "@comms/shared/api/relayClient";
import { useIdentityQuery } from "@comms/shared/api/hooks";
import { useRelayAutoHeal } from "@comms/shared/api/useRelayAutoHeal";
import { useDeferredStartup } from "@comms/shared/hooks/useDeferredStartup";
import { useWebviewScrollBoundaryLock } from "@comms/shared/hooks/useWebviewScrollBoundaryLock";
import { joinChannel } from "@comms/shared/api/tauri";
import type { ChannelVisibility, SearchHit } from "@comms/shared/api/types";
import { ChannelNavigationProvider } from "@comms/shared/context/ChannelNavigationContext";
import { MainInsetProvider } from "@comms/shared/layout/MainInsetContext";
import { chromeCssVarDefaults } from "@comms/shared/layout/chromeLayout";
import { cn } from "@comms/shared/lib/cn";
import { hasPrimaryShortcutModifier } from "@comms/shared/lib/platform";
import { useMessageDeepLinks } from "@comms/shared/useMessageDeepLinks";
import { SidebarInset, SidebarProvider } from "@comms/shared/ui/sidebar";
import { RelayConnectionOverlay } from "@comms/app/RelayConnectionOverlay";
import { useSidebarRelayConnectionCard } from "@comms/features/sidebar/ui/useSidebarRelayConnectionCard";
import { AppShellTrayMenu } from "@comms/app/useAppShellTrayMenu";

const LazySettingsScreen = React.lazy(async () => {
  const module = await import("@comms/features/settings/ui/SettingsScreen");
