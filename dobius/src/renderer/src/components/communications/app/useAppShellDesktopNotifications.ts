...
import {
  ...
  toSearchHit,
} from "@comms/app/AppShell.helpers";
import { getThreadReference } from "@comms/features/messages/lib/threading";
import { hasMentionForEvent } from "@comms/features/notifications/lib/shouldNotify";
import type { NotificationSettings } from "@comms/features/notifications/hooks";
import {
  listenForDesktopNotificationActions,
  requestDockBounce,
  revealDesktopAppWindow,
  sendDesktopNotification,
} from "@comms/features/notifications/lib/desktop";
import {
  formatNotificationTitle,
  truncateNotificationBody,
} from "@comms/features/notifications/lib/notificationFormat";
import {
  playNotificationSound,
  resolveSlotSound,
} from "@comms/features/notifications/lib/sound";
...
export function useAppShellDesktopNotifications({
  ...
  openSearchHit: (
    hit: import("@comms/shared/api/types").SearchHit,
  ) => Promise<unknown>;
  pubkey?: string;
}) {
  ...
  const handleDesktopNotificationAction = React.useEffectEvent(
    async (
      target: import("@comms/features/notifications/lib/desktop").DesktopNotificationTarget,
    ) => {
