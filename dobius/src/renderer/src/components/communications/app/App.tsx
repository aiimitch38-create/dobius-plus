...
import { router } from "@comms/app/router";
import {
  completeCommunityViewTransition,
  replaceCommunityDestinationRoute,
} from "@comms/app/communityViewTransition";
import { deriveShellRoute } from "@comms/app/AppShell.helpers";
import { ThemeGrainientBackground } from "@comms/app/ThemeGrainientBackground";
import { useReloadShortcut } from "@comms/app/useReloadShortcut";
import { KnownAgentPubkeysProvider } from "@comms/features/agents/useKnownAgentPubkeys";
import { useAppOnboardingState } from "@comms/features/onboarding/hooks";
import { useMachineOnboardingState } from "@comms/features/onboarding/machineOnboarding";
import {
...
} from "@comms/features/onboarding/communityOnboarding";
import { CommunityOnboardingFlow } from "@comms/features/onboarding/ui/CommunityOnboardingFlow";
import {
  MachineOnboardingFlow,
  type MachineOnboardingPage,
  type PostOnboardingNavigation,
} from "@comms/features/onboarding/ui/MachineOnboardingFlow";
import { OnboardingFlow } from "@comms/features/onboarding/ui/OnboardingFlow";
import { PendingInviteGate } from "@comms/features/onboarding/ui/PendingInviteGate";
import { KeyringLockedScreen } from "@comms/features/onboarding/ui/KeyringLockedScreen";
import { RelaunchRequiredScreen } from "@comms/features/onboarding/ui/RelaunchRequiredScreen";
import { ResetFailedScreen } from "@comms/features/onboarding/ui/ResetFailedScreen";
import { useCommunityInit } from "@comms/features/communities/useCommunityInit";
import { useNestNotifications } from "@comms/features/communities/useNestNotifications";
...
} from "@comms/features/communities/addCommunityPrefill";
import { WelcomeSetup } from "@comms/features/communities/ui/WelcomeSetup";
import { CommunityApplyErrorScreen } from "@comms/features/communities/ui/CommunityApplyErrorScreen";
...
// The Dobius mark for the loading gates. It is a single image with a CSS
// pulse, so it paints on the very first frame — before scripting starts —
// avoiding a blank flash on hard reload, and the animation runs on the
// compositor rather than the main thread that boot work is busy with.
function MarkLoader({
  ariaLabel,
  className,
  ...
  return (
    <div className={cn("relative", tintClassName, className)}>
      <FuzzyLogo ariaLabel={ariaLabel} className="h-full w-full" />
    </div>
  );
}

// Cold boot gate: the theme-adaptive grainient background with a single
// centered Dobius mark breathing over it. Replaces the old "Setting up your
...
function AppLoadingGate() {
  return (
    ...
    >
      ...
      <LoadingMark className="relative z-10 h-auto w-28" />
    </div>
  );
}
