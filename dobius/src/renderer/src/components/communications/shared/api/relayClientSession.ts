...
import type { PresenceStatus, RelayEvent } from "@comms/shared/api/types";
import {
  KIND_STREAM_MESSAGE,
...
} from "@comms/shared/api/relayClientShared";
import {
...
} from "@comms/shared/api/relayChannelFilters";
import {
  clearClosedRetry,
  handleRelayClosed,
  handleSubscriptionEose,
  prepareSubscriptionEvent,
} from "@comms/shared/api/relayClosedRecovery";
import { replayLiveSubscriptions } from "@comms/shared/api/relayReconnectReplay";
import {
  activateRateLimit,
  parseRateLimitHint,
  waitForRateLimit,
} from "@comms/shared/api/relayRateLimitGate";
import {
  fetchChunkedHistory,
  requestFirstEventGated,
  requestHistoryGated,
} from "@comms/shared/api/relayGateBoundary";
import { RelayConnectionStateEmitter } from "@comms/shared/api/relayConnectionStateEmitter";
import {
...
} from "@comms/shared/api/relayReconnectPolicy";
import { RelayReconnectWaiters } from "@comms/shared/api/relayReconnectWaiters";
import { RelayStallWatchdog } from "@comms/shared/api/relayStallWatchdog";
import {
  closeWebSocket,
  openWebSocket,
  sendOnWebSocket,
} from "@comms/shared/api/relayWebSocketClose";
import { buildThreadReferenceTags } from "@comms/features/messages/lib/threading";
...
export class RelayClient {
  ...
  private async connect() {
    ...
    try {
      ...
      const wsId = await openWebSocket(this.relayUrl, handleMessage);
