...
import { isMacPlatform } from "@comms/shared/lib/platform";
...
/**
 * The Dobius themes ship with a fixed neutral accent (the GitHub black/white
 * foreground) rather than a user-selectable accent color. When a Dobius theme is
...
/**
 * Resolve the accent to actually apply for a theme: Dobius themes are pinned to
...
/**
 * Toggle the opaque Dobius sidebar-gradient marker. This is always safe to apply
...
function applyBuzzSidebar(themeName: string) {
  ...
  if (isBuzzTheme(themeName)) {
    ...
    // Keep the concrete Dobius variant on the root as well as the generic
    ...
    // Leaving Dobius: drop translucency synchronously here too. Going *opaque*
 ...
 * Dobius → non-Dobius toggle can fire two overlapping calls whose awaits resolve out
 ...
 * translucency — otherwise the earlier Dobius call could re-add
 * `data-buzz-translucent` after the later non-Dobius call already cleared it,
 * leaving the window transparent under a non-Dobius theme.
 */
let buzzVibrancyRequest = 0;

/**
 * Whether the native vibrancy layer is confirmed installed for a Dobius theme.
...
/** The native layer does not need rebuilding when Dobius only changes mode. */
 ...
 *  2. the Dobius sidebar marker + gradient vars are applied (`data-buzz-sidebar`,
 ...
 * through; enabling it before the Dobius gradient vars are installed would flash a
 ...
 * behind it:
 *
 * - Entering Dobius (macOS): install the vibrancy layer first (await the IPC),
 ...
 * - Leaving Dobius: translucency was already removed synchronously in
 ...
 * these platforms fall back to the opaque Dobius gradient.
 *
...
async function applyBuzzVibrancy(themeName: string) {
  ...
  // Dobius Light and Dobius Dark use the same native material. Rebuilding the
  ...
  try {
    ...
    // — but only if `applyBuzzSidebar` has already installed the Dobius gradient
...
function applyCachedVars(): string | null {
  try {
    ...
    // Pin Dobius themes to the neutral accent here too, matching applyTheme.
    // Otherwise a cached Dobius theme + non-neutral stored accent flashes the
...
async function applyTheme(
  ...
  // The Dobius gradient vars are now installed. If the vibrancy layer already
  ...
  // new theme for a frame — the flicker seen when switching to Dobius. Dobius
...
export function ThemeProvider({
  ...
  const [followSystem, setFollowSystemState] = useState<boolean>(() => {
    ...
    // Fresh profiles (no saved theme) default to System mode so the Dobius
  ...
  // changes. applyTheme already applies the (Dobius-neutral-aware) accent in the
