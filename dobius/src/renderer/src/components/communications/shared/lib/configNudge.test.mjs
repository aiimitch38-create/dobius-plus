...
test("extractConfigNudge returns null when no sentinel present", () => {
  assert.equal(
    extractConfigNudge("**Iris** needs configuration before it can respond."),
    null,
  );
});
...
test("extractConfigNudge parses env_key requirement", () => {
  ...
  const content = [
    "**Iris** needs configuration before it can respond:",
    "- set `ANTHROPIC_API_KEY` in Edit Agent → Environment variables",
    "",
    "Open Edit Agent in the Dobius app to set these.",
    "",
    "```buzz:config-nudge",
    JSON.stringify(payload),
    "```",
...
test("stripConfigNudgeSentinel strips the sentinel block", () => {
  const prose = "**Iris** needs configuration.\n\nOpen Edit Agent.";
  const payload = {
    agent_name: "Iris",
    agent_pubkey: FIZZ_PUBKEY,
    requirements: [],
...
function makeNudgeBody(agentPubkey) {
  ...
  return `**Iris** needs configuration.\n\n\`\`\`buzz:config-nudge\n${JSON.stringify(payload)}\n\`\`\``;
}
