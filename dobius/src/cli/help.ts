...
export function formatFlagHelp(flag: string): string {
  const helpByFlag: Record<string, string> = {
    ...
    engine: '--engine <name>       Agent engine: claude or codex',
    account: '--account <id>       Existing Dobius-managed OAuth account id',
    model: '--model <id>           Model id for the selected agent engine',
